            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑设计题目</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="jiaoshiId" name="jiaoshiId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>项目类型</label>
                                        <select id="shejitimuTypesSelect" name="shejitimuTypes" class="form-control">
                                        </select>
                                    </div>
                                        <input id="shejitimuZancheng" type="hidden" value="0" name="shejitimuZancheng" class="form-control"
                                               onchange="shejitimuZanchengChickValue(this)"  placeholder="赞成票">
                                        <input id="shejitimuFandui" type="hidden" value="0"  name="shejitimuFandui" class="form-control"
                                               onchange="shejitimuFanduiChickValue(this)"  placeholder="反对票">

                                        <input type="hidden" value="1" id="zhuangtaiTypesSelect" name="zhuangtaiTypes" class="form-control">
                                    <div class="form-group  col-md-6">
                                        <label>项目内容</label>
                                        <input id="shejitimuContentupload" name="file" type="file">
...
<script>
    ...
    function upload() {
        ...
        $('#shejitimuContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('shejitimuContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#jiaoshiId") !=null){
            ...
            httpJson("shejitimu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationShejitimutypesSelect(){
            var shejitimuTypesSelect = document.getElementById('shejitimuTypesSelect');
            if(shejitimuTypesSelect != null && shejitimuTypesOptions != null  && shejitimuTypesOptions.length > 0 ){
                for (var i = 0; i < shejitimuTypesOptions.length; i++) {
                    shejitimuTypesSelect.add(new Option(shejitimuTypesOptions[i].indexName,shejitimuTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(shejitimuTypesSelect != null && shejitimuTypesOptions != null  && shejitimuTypesOptions.length > 0 ) {
            for (var i = 0; i < shejitimuTypesOptions.length; i++) {
                if (shejitimuTypesOptions[i].codeIndex == ruleForm.shejitimuTypes) {//下拉框value对比,如果一致就赋值汉字
                        shejitimuTypesSelect.options[i].selected = true;
                }
            }
        }
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.shejitimuContent != null && ruleForm.shejitimuContent != 'null' && ruleForm.shejitimuContent != '' && $("#shejitimuContentupload").length>0) {
    ...
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#shejitimuContentupload").length>0) {
            var shejitimuContentEditor = UE.getEditor('shejitimuContentEditor');
            if (shejitimuContentEditor.hasContents()) {
                $('#shejitimuContent-input').attr('value', shejitimuContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function shejitimuZanchengChickValue(e){
        ...
        function shejitimuFanduiChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[1-9][0-9]*$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                shejitimuName: "required",
                jiaoshiId: "required",
                shejitimuTypes: "required",
                insertTime: "required",
                shejitimuZancheng: "required",
                shejitimuFandui: "required",
                zhuangtaiTypes: "required",
                shejitimuContent: "required",
            },
            messages: {
                shejitimuName: "题目名称不能为空",
                jiaoshiId: "发布教师不能为空",
                shejitimuTypes: "项目类型不能为空",
                insertTime: "发布时间不能为空",
                shejitimuZancheng: "赞成票不能为空",
                shejitimuFandui: "反对票不能为空",
                zhuangtaiTypes: "审核状态不能为空",
                shejitimuContent: "项目内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshejitimu = window.sessionStorage.getItem("addshejitimu");
        if (addshejitimu != null && addshejitimu != "" && addshejitimu != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("shejitimu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationShejitimutypesSelect();
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
        if(window.sessionStorage.getItem('addshejitimu') != null && window.sessionStorage.getItem('addshejitimu') !=""){//新增查询,修改不查
            // 查询当前登录账户的信息
            var userId = window.sessionStorage.getItem('userId');
