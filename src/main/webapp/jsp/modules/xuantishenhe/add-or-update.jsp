    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑选题审核</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑选题审核</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">选题审核信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="shejitimuId" name="shejitimuId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>审核状态</label>
                                        <select id="xuantishenheTypesSelect" name="xuantishenheTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#shejitimuId") !=null){
            ...
            httpJson("xuantishenhe/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addxuantishenhe');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationXuantishenhetypesSelect(){
            var xuantishenheTypesSelect = document.getElementById('xuantishenheTypesSelect');
            if(xuantishenheTypesSelect != null && xuantishenheTypesOptions != null  && xuantishenheTypesOptions.length > 0 ){
                for (var i = 0; i < xuantishenheTypesOptions.length; i++) {
                    xuantishenheTypesSelect.add(new Option(xuantishenheTypesOptions[i].indexName,xuantishenheTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(xuantishenheTypesSelect != null && xuantishenheTypesOptions != null  && xuantishenheTypesOptions.length > 0 ) {
            for (var i = 0; i < xuantishenheTypesOptions.length; i++) {
                if (xuantishenheTypesOptions[i].codeIndex == ruleForm.xuantishenheTypes) {//下拉框value对比,如果一致就赋值汉字
                        xuantishenheTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                xuantishenheTypes: "required",
            },
            messages: {
                yonghuId: "选择学生不能为空",
                jiaoshiId: "选择题目不能为空",
                shejitimuId: "发布教师不能为空",
                xuantishenheTypes: "审核状态不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addxuantishenhe = window.sessionStorage.getItem("addxuantishenhe");
        if (addxuantishenhe != null && addxuantishenhe != "" && addxuantishenhe != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("xuantishenhe/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationXuantishenhetypesSelect();
            <!--  初始化级联表的下拉框  -->
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $(".aaaaaa").remove();
        if(window.sessionStorage.getItem('addxuantishenhe') != null && window.sessionStorage.getItem('addxuantishenhe') !=""){//新增查询,修改不查
            // 查询当前登录账户的信息
            var userId = window.sessionStorage.getItem('userId');
